...
# Now on REALm — Estate Brown デザインシステム用フォント（全ページ共通）
FONTS_VIGIL = (
...
# ブランドマーク HTML スニペット
BRAND_NAV  = '<em class="dot-r">●</em>Now on <em class="brand-ai">REAL</em><span class="brand-r">m</span>'
BRAND_FULL = 'Now on <span style="color:var(--brown);font-style:normal;">REAL</span><span style="color:var(--mute);">m</span>'
...
<div class="archive">
  ...
  <h2>About</h2>
  <span class="jp">Now on REALmについて</span>
</div>
<div class="about">
  <div class="card-about">
    <h3>NOW ON AIr — Real Estate Morning Intelligence</h3>
  ...
  <div class="left"><b>Now on <span class="brand-ai">REAL</span>m</b> · Real Estate Morning Intelligence · 山中秀斗 / TREPRO</div>
  <div class="right">{today_str} · BUILD {dt.strftime('%Y%m%d')}.0715</div>
</div>"""

    html = _head(
        "Now on REALm — Real Estate Morning Intelligence",
...
{links_html}
    ...
    html = _head(
        f"{date_str} Morning Dispatch — Now on REALm",
        f"{date_str} の Now on REALm モーニングディスパッチ — {len(articles)}本の不動産業界ニュース。",
        "../../assets/vigil.css",
        FONTS_VIGIL,
    ) + DAILY_CSS + f"""
{dispatch}
